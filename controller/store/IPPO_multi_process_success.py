...
class IPPO:
    def __init__(self, args, env, device, model_path=None):
        ...
        self.actors = [UNet() for _ in range(self.num_agent)]
        self.critics = [CNNCritic() for _ in range(self.num_agent)]
    ...
    def train(self, trained_iterations, save_folder):
        ...
        while i_so_far <= trained_iterations:
            try:
                ...
                batch_states_all, batch_actions_all, batch_log_probs_all, batch_rewards_all, batch_next_states_all, batch_advantages_all, batch_returns_all, batch_values_all = self.parallel_roll_out_mp(num_processes=2)
    ...
    def roll_out_worker(self, process_id, batch_size):
        """
        Hàm roll_out được chạy trong một process riêng biệt
        
        Args:
            process_id: ID của process
            batch_size: Kích thước batch cho process này
            
        Returns:
            Dữ liệu thu thập được từ process này
        """
        # Tạo file log riêng cho process này
        file_log = open(f'log_process_{process_id}.csv', 'w', newline='')
        ...
        while True:
            ...
            print(f"Process {process_id} bắt đầu roll_out, Number sample: {t}")
            while True:
            ...
            for id in range(self.num_agent):
                ...
                returns, advantages, values = self.cal_rt_adv(
                    id, 
                    torch.FloatTensor(np.array(states[id])).to(self.device), 
                    torch.FloatTensor(np.array(rewards[id])).to(self.device), 
                    torch.FloatTensor(np.array(next_states[id])).to(self.device), 
                    torch.FloatTensor(np.array(terminals[id])).to(self.device)
                )
                ...
                batch_advantages[id].extend(advantages.detach().cpu().numpy())
                batch_returns[id].extend(returns.detach().cpu().numpy())
                batch_values[id].extend(values.detach().cpu().numpy())
        ...
        for id in range(self.num_agent):
            ...
            selected_num = int(batch_size / 2.0)
            random_num = batch_size - selected_num
            indices = np.concatenate((
                indices[-selected_num:], 
                np.random.choice(len(batch_rewards[id]) - selected_num, size=random_num, replace=False)
            ))
            
            processed_data[id] = {
                'states': np.array(batch_states[id])[indices],
                'actions': np.array(batch_actions[id])[indices],
                'log_probs': np.array(batch_log_probs[id])[indices],
                'rewards': np.array(batch_rewards[id])[indices],
                'next_states': np.array(batch_next_states[id])[indices],
                'advantages': np.array(batch_advantages[id])[indices],
                'returns': np.array(batch_returns[id])[indices],
                'values': np.array(batch_values[id])[indices]
            }
        
        file_log.close()
        return processed_data, f'log_process_{process_id}.csv'
    
    # Hàm worker được chạy trong mỗi process
    def process_worker(self, proc_id, batch_size, result_dict):
        # Thực hiện roll_out và lưu kết quả vào dict được chia sẻ
        data, log_file = self.roll_out_worker(proc_id, batch_size)
        result_dict[proc_id] = (data, log_file)

    def parallel_roll_out(self, num_processes=1):
        """
        Chạy nhiều process roll_out song song và kết hợp kết quả sử dụng Process thay vì Pool.
        
        Args:
            num_processes: Số lượng process song song
            
        Returns:
            Dữ liệu roll_out đã được kết hợp
        """
        
        # Chuẩn bị các tham số
        batch_size_per_process = self.batch_size // num_processes
        ...
        for i in range(num_processes):
            p = mp.Process(
                ...
                args=(i, batch_size_per_process, result_dict)
            )
            processes.append(p)
            p.start()
        ...
        for i in range(num_processes):
            if i in result_dict:
                data, log_file = result_dict[i]
                collected_data.append(data)
                log_files.append(log_file)
        

        
        # Kết hợp logs từ tất cả các process
        self.file_log_all = open('log_cur.csv', 'w', newline='')
        self.writer_log_all = csv.writer(self.file_log_all)
        
        for log_file in log_files:
            with open(log_file, 'r') as f:
                self.writer_log_all.writerows(csv.reader(f))
            os.remove(log_file)  # Xóa file log tạm
        ...
        for data in collected_data:
            for id in range(self.num_agent):
                if id in data:
                    agent_data = data[id]
                    batch_states[id].extend(agent_data['states'])
                    batch_actions[id].extend(agent_data['actions'])
                    batch_log_probs[id].extend(agent_data['log_probs'])
                    batch_rewards[id].extend(agent_data['rewards'])
                    batch_next_states[id].extend(agent_data['next_states'])
                    batch_advantages[id].extend(agent_data['advantages'])
                    batch_returns[id].extend(agent_data['returns'])
                    batch_values[id].extend(agent_data['values'])
        
        # Chuyển đổi thành tensors
        for id in range(self.num_agent):
            if batch_rewards[id]:  # Kiểm tra nếu danh sách không rỗng
                batch_rewards[id] = torch.FloatTensor(np.array(batch_rewards[id])).to(self.device)
                batch_states[id] = torch.FloatTensor(np.array(batch_states[id])).to(self.device)
                batch_actions[id] = torch.FloatTensor(np.array(batch_actions[id])).to(self.device)
                batch_next_states[id] = torch.FloatTensor(np.array(batch_next_states[id])).to(self.device)
                batch_log_probs[id] = torch.FloatTensor(np.array(batch_log_probs[id])).to(self.device)
                batch_advantages[id] = torch.FloatTensor(np.array(batch_advantages[id])).to(self.device)
                batch_returns[id] = torch.FloatTensor(np.array(batch_returns[id])).to(self.device)
                batch_values[id] = torch.FloatTensor(np.array(batch_values[id])).to(self.device)
        
        return (batch_states, batch_actions, batch_log_probs, batch_rewards, 
                batch_next_states, batch_advantages, batch_returns, batch_values)
    ...
    def parallel_roll_out_mp(self, num_processes):
        ...
        for i in range(num_processes):
            p = Process(
                ...
                args=(
                    i, result_queue, actor_states, critic_states, 
...
# Hàm worker độc lập (không phải phương thức của lớp)
def standalone_worker(proc_id, result_queue, actor_states, critic_states, num_agent, batch_size, 
    ...
    from rl_env.WRSN import WRSN
    proc_env = WRSN(scenario_path="physical_env/network/network_scenarios/_20targets_57sensors.yaml",
                   agent_type_path="physical_env/mc/mc_types/default.yaml",
    ...
    # Hàm tính returns và advantages
