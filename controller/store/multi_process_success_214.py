    ...
    def train(self, trained_iterations, save_folder):
        ...
        while i_so_far <= trained_iterations:
            try:
                ...
                batch_states_all, batch_actions_all, batch_log_probs_all, batch_rewards_all, batch_next_states_all, batch_advantages_all, batch_returns_all, batch_values_all = self.parallel_roll_out_mp(num_processes=4)
                ...
                for id in range(self.num_agent):
                    ...
                    for _ in range(self.n_updates_per_iteration):
                        ...
                        for start in range(0, self.batch_size, self.minibatch_size):
                            ...
                            # print(f"mb_inds type: {type(mb_inds)}, shape: {mb_inds.shape if hasattr(mb_inds, 'shape') else 'N/A'}")
                            # print(f"batch_states type: {type(batch_states)}, shape: {batch_states.shape if hasattr(batch_states, 'shape') else 'N/A'}")
